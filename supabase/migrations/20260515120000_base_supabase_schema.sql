...
CREATE TABLE public.orders (
    ...
    session_token TEXT, -- Added for guest identification
...
DROP POLICY IF EXISTS "Admin can update profiles in same restaurant." ON public.profiles;
...
CREATE POLICY "Everyone can view orders." ON public.orders FOR SELECT USING (
    -- Access if session_id is provided and valid
    EXISTS (
...
CREATE POLICY "Everyone can update orders." ON public.orders FOR UPDATE USING (
