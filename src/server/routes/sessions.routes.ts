...
router.get("/restaurants/:restId/dining-sessions", authenticateJWT, async (req, res) => {
...
router.get("/dining-sessions/:id/orders", authenticateJWT, async (req, res) => {
...
router.post("/dining-sessions/:id/settle", authenticateJWT, async (req, res) => {
...
router.patch("/dining-sessions/:id", authenticateJWT, async (req, res) => {
  const { data, error } = await supabaseAdmin
    .from('dining_sessions')
    .update(req.body)
    .eq('id', req.params.id)
