...
app.post("/api/public/orders/:id/mark-paid", async (req, res) => {
  ...
  // Idempotency: check if the order is already marked paid to avoid overwriting paid_at
...
app.post("/api/restaurants/:restId/staff", authenticateJWT, async (req, res) => {
  ...
  try {
    ...
    // Save to fallback DB profiles list too
    const db = loadFallbackDB();
...
app.put("/api/restaurants/:restId/staff/:staffId", authenticateJWT, async (req, res) => {
  ...
  try {
    ...
    if (isPrimary) {
      ...
      // Secondary workspace: update role and custom permissions on restaurant_users mapping
...
app.delete("/api/restaurants/:restId/staff/:staffId", authenticateJWT, async (req, res) => {
  ...
  try {
    ...
    if (isPrimary) {
      ...
      // Erase mapping from local fallback DB
...
async function start() {
  ...
  // Background translation job definitions
  async function translateTextWithGemini(text: string, targetLang: string): Promise<string | null> {
